...
def argparser():
	...
	parser.add_argument("--epoch_num",  type = int,default = 1,help = "Number of epoch to train.")
	parser.add_argument("--embedding_size", type = int, default = 300, help = "Word Embedding Vector dimension size, default = 300")
	parser.add_argument("--hidden_size", type = int, default = 128, help = "Hidden size of RNN. default = 300")
	parser.add_argument("--model_path", type = str, default = "model", help = "The path of encoder and decoder models.")
	parser.add_argument("--srcLang", type = str, default = "wasted", required = False, help = "The language of source.")
	parser.add_argument("--tgtLang", type = str, default = "nor", required = False, help = "The language of target.")
...
def trainiters(pairs, encoder, decoder, epoch_number, model_path, train_pairs_seed = 0, print_every = 1000, plot_every = 1000, learning_rate = .01):
	...
	train_pairs, test_pairss = train_test_split(pairs, test_size = 0.3, random_state = train_pairs_seed)
	val_pairs, test_pairs = train_test_split(test_pairss, test_size=0.3, random_state=train_pairs_seed)
	...
	val_pairs = [tensorsFromPair(index, len(val_pairs), pair) for index, pair in enumerate(val_pairs)]
...
if __name__ == "__main__":
	...
	input_lang, output_lang, pairs = loader.prepareData(config.srcLang, config.tgtLang)
